 ...
 *
 */
@ManagedService(path = "/notification/{room: [a-zA-Z_0-9]*}")
public class YoryoNotificationHandler {
    private final Logger logger = LoggerFactory.getLogger(YoryoNotificationHandler.class);

    private final ConcurrentHashMap<String, String> users = new ConcurrentHashMap<String, String>();

    public final static String CHAT = "/notification/";
    ...
    public ChatProtocol onReady(final AtmosphereResource r) {
        ...
        ChatProtocol chatProtocol = new ChatProtocol(users.keySet(), getRooms(factory.lookupAll()));
        return chatProtocol;
    }
    ...
    private static Collection<String> getRooms(Collection<Broadcaster> broadcasters) {
        ...
        for (Broadcaster broadcaster : broadcasters) {
            if (!("/*".equals(broadcaster.getID()))) {
                String room = broadcaster.getID().split("/")[2];
                if(!YoryoNotificationConstants.CURRENT_CHAT_ROOMS.containsKey(room)) {
                    YoryoNotificationConstants.CURRENT_CHAT_ROOMS.put(room, room);
                }
                result.add(room);
            }
        };
